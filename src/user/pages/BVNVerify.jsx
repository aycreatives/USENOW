...
function BVNVerify() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedVerify === "bvn" && (!bvn || bvn.length !== 11)) {
      toast.error("Please enter a valid 11-digit BVN");
      return;
    }

    if (selectedVerify === "phone" && (!phone || phone.length !== 11)) {
    ...
    if (selectedVerify === "bvn") {
      payload.bvn = bvn;
    } else if (selectedVerify === "phone") {
      payload.phone = phone;
    }
    setLoading(true);
    try {
      ...
      viewSlip(response.data?.result?.data, selectedSlip);
