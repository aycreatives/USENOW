...
export default function ValidationHistory() {
  ...
  const apiLink = `${config.apiBaseUrl}${config.endpoints.validationHistory}${userId}`;
  ...
  const filteredTransactions = apiData.filter((transaction) => {
    ...
    const passesSearchFilter =
      ...
      transaction.nin?.toLowerCase().includes(searchStr) ||
      ...
      {!loading && sortedTransactions.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center p-8">
          <Empty
            ...
            description={<span>No Validation history records found.</span>}
          />
        </div>
      ) : null}
      ...
      >
        {selectedTransaction && (
          <div className="space-y-4 p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <p className="text-sm font-medium text-gray-500">Slip Type</p>
              <p className="text-sm text-gray-900">
                {selectedTransaction.slipType || "N/A"}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <p className="text-sm font-medium text-gray-500">NIN</p>
              <p className="text-sm text-gray-900">
                {selectedTransaction.nin || "N/A"}
              </p>
            </div>
          </div>
        )}
